// Shared brand utilities for Cloud Functions
// Keep in sync with src/lib/brand-utils.ts
...
export function resolveOrgColors(branding?: { primaryColor?: string; secondaryColor?: string } | null): ResolvedBrandColors {
  ...
  const secondary = branding?.secondaryColor && isValidHex(branding.secondaryColor) ? branding.secondaryColor : primary;
  const primaryLight = lightenHex(primary, 0.92);
  const textOnPrimary = getContrastColor(primary);
  return { primary, primaryLight, secondary, textOnPrimary };
}
