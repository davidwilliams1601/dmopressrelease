...
type SummaryResult =
  | { success: true; data: GeneratePressReleaseSummaryOutput }
...
export async function generatePressReleaseSummary(
  ...
  try {
    ...
    console.error('[AI Summary] Stack:', error.stack);
    console.error('[AI Summary] GEMINI_API_KEY set:', !!process.env.GEMINI_API_KEY);
